...
def get_batch_random(data_train, data_labels, batch_size):
    ...
    for i in range(batch_size):
        ...
        # triplets[0][i,:,:,0] = data_train[cat][pos_ind,0,:,:,0]
        # triplets[1][i,:,:,0] = np.rot90(data_train[cat][pos_ind,0,:,:,0])
        # triplets[0][i,:,:,1] = data_train[cat][pos_ind,0,:,:,0]
        # triplets[1][i,:,:,1] = np.rot90(data_train[cat][pos_ind,0,:,:,0])
        # triplets[0][i,:,:,2] = data_train[cat][pos_ind,0,:,:,0]
        # triplets[1][i,:,:,2] = np.rot90(data_train[cat][pos_ind,0,:,:,0])
...
def get_batch_hard(data, data_labels, draw_batch_size, hard_batchs_size, norm_batchs_size, network, s="train"):
    #Step 1 : pick a random batch to study
    studybatch = get_batch_random(data, data_labels, draw_batch_size)
